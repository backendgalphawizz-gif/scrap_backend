@php
    $testingDetail = \App\Model\SchemeInspectionBody::where('application_id', $application->id)->first();
@endphp

<div class="container-fluid">
    <div class="row">
        <div class="col-lg-12">
            <h4>For Inspection Body Applicant</h4>
        </div>
    ...
    <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
        ...
        <thead>
            <tr>
                <th colspan="3">Scope of inspection for which accreditation is sought.</th>
            </tr>
            <tr>
                <th>{{ \App\CPU\translate('Type of Inspection What do you consider to be the type of your Inspection Body, as defined in ISO/IEC 17020? Type A, Type B and/or Type C')}}</th>
                <th>{{ \App\CPU\translate('Inspection Category')}}</th>
                <th>{{ \App\CPU\translate('Inspection Field (And Sub-fields) such as: product design, products (specified as materials or equipment) Installations, plant, premises, processes, services and surveys')}}</th>
                <th>{{ \App\CPU\translate('Type and Range of Inspection e.g., In-service inspection or inspection of new products')}}</th>
                <th>{{ \App\CPU\translate('Stage of Inspection')}}</th>
                <th>{{ \App\CPU\translate('Methods / Standards / Specifications / Techniques such as:EC directives, regulations, standards, specifications, internal procedures')}}</th>
                <th>{{ \App\CPU\translate('Remark')}}</th>
                <th>{{ \App\CPU\translate('Remark By')}}</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($testingDetail->scope_of_inspection as $scope)
                <tr>
                    <td>{{ $scope['type_of_inspection'] ?? '-' }}</td>
    ...
    <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
        ...
        <thead>
            <tr>
                <th colspan="3">For whom does the inspection body undertake inspection?</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td><input type="checkbox" name="undertake_inspection" {{$testingDetail->undertake_inspection == 'Own organization' ? 'checked' : ''}} disabled> Own organization</td>
                <td><input type="checkbox" name="undertake_inspection" {{$testingDetail->undertake_inspection == 'Parent organization' ? 'checked' : ''}} disabled> Parent organization</td>
                <td><input type="checkbox" name="undertake_inspection" {{$testingDetail->undertake_inspection == 'Other organization' ? 'checked' : ''}} disabled> Other organization</td>
            </tr>
        </tbody>
    </table>
    ...
    <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
        ...
        <thead>
            <tr>
                <th colspan="3">What do you consider to be the type of your Inspection Body, as defined in ISO/IEC 17020?</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td><input type="checkbox" name="type_inspection_body" {{$testingDetail->type_inspection_body == 'Type A' ? 'checked' : ''}} disabled> Type A</td>
                <td><input type="checkbox" name="type_inspection_body" {{$testingDetail->type_inspection_body == 'Type B' ? 'checked' : ''}} disabled> Type B</td>
                <td><input type="checkbox" name="type_inspection_body" {{$testingDetail->type_inspection_body == 'Type C' ? 'checked' : ''}} disabled> Type C</td>
            </tr>
        </tbody>
    </table>
    ...
    <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
        ...
        <thead>
            <tr>
                <th colspan="3">Do you perform inspection outside USA?</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td><input type="checkbox" name="inspection_outside_usa" {{$testingDetail->inspection_outside_usa == 1 ? 'checked' : ''}} disabled> Yes</td>
                <td><input type="checkbox" name="inspection_outside_usa" {{$testingDetail->inspection_outside_usa == 0 ? 'checked' : ''}} disabled> No</td>
            </tr>
            <tr>
                <td>If yes state, the countries in which inspections performed</td>
                <td>{{$testingDetail->other_inspection_country }}</td>
            </tr>
        </tbody>
    </table>
    ...
    <div class="tabl-responsive">
        <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
            ...
            <thead>
                <tr>
                    <th colspan="2">
                        Type of calibration for the inspection body equipment:
                    </th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>
                        <label for=""><input type="checkbox" disabled {{ $testingDetail->inspection_equipment['external']??'' == 0 ? 'checked' : '' }}> External</label>
                    </td>
                    <td>
                        <label for=""><input type="checkbox" disabled {{ $testingDetail->inspection_equipment['internal']??'' == 0 ? 'checked' : '' }}> Internal</label>
                    </td>
                    <td>
                        <label for="">Remark: {{ $testingDetail->inspection_equipment['remark']??'' }}</label>
                    </td>
                    <td>
                        <label for="">Remark By: {{ $testingDetail->inspection_equipment['remark_by']??'' }}</label>
                    </td>
                </tr>
            </tbody>
        </table> 
    ...
    <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
        ...
        <thead>
            <tr>
                ...
                <th>{{ \App\CPU\translate('Brief Description of Measurement and Equipment Used')}}</th>
                <th>{{ \App\CPU\translate('remark')}}</th>
