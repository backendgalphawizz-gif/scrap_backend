    ...
    <div class="content container-fluid">
        ...
        <div class="card">
                ...
                <div class="d-flex flex-wrap w-100 gap-3 align-items-center">
                    ...
                    <form action="" method="GET" class="mb-0">
                        ...
                        <div class="input-group input-group-merge input-group-custom">
                            ...
                            <input id="datatableSearch_" value="{{ $search }}" type="search" name="search" class="form-control d-none" placeholder="{{ \App\CPU\translate('search_by_order_id')}}" aria-label="Search orders" required>
                            <button type="submit" class="btn btn--primary d-none">{{ \App\CPU\translate('search')}}</button>
                        </div>
                        <!-- End Search -->
                    </form>
                    <div>
                        ...
                        <ul class="dropdown-menu dropdown-menu-right">
                            ...
                            <li>
                                <a class="dropdown-item" href="{{ route('admin.report.order-report-excel', ['date_type'=>request('date_type'), 'seller_id'=>request('seller_id'), 'from'=>request('from'), 'to'=>request('to'), 'search'=>request('search'), 'type' => 'csv']) }}">
            ...
            <div class="table-responsive mt-2">
                <table id="datatable" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};" class="report-order-list table __table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100">
                    <thead class="thead-light thead-50 text-capitalize">
                        <tr>
                            <th>{{\App\CPU\translate('SL')}}</th>
                            <th>{{\App\CPU\translate('Order ID')}}</th>
...
@push('script_2')
    ...
    <script>
        ...
        var orderTable = $(".report-order-list").DataTable({
            // dom: 'frltip',
            ...
            order: [[0, "desc"]],
            ajax: {
                url:"{{ route('admin.report.order.paginate') }}",
                data: function(data) {
                    data.seller_id = $('select[name=seller_id]').val(),
                    data.date_type = $('select[name=date_type]').val(),
                    data.from = $('input[name=from]').val(),
                    data.to = $('input[name=to]').val(),
                    data.search['value'] = $('#datatable_filter').find('input[type=search]').val()
                }
            ...
            columns: [
                ...
                {'name': 'orders.id'},
                {'name': 'order_amount'},
                {'name': 'details_sum_discount'},
                {'name': 'discount_amount'},
                {'name': 'shipping_cost'},
                {'name': 'details_sum_tax'},
