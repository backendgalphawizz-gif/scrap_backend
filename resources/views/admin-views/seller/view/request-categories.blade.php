                        ...
                        <div class="px-3 py-4">
                            <div class="row align-items-center">
                                <div class="col-lg-4 mb-3 mb-lg-0">
                                    <h5 class="mb-0 text-capitalize d-flex gap-1 align-items-center">{{ \App\CPU\translate('Categories Request')}}
                                ...
                                <div class="col-md-6 col-lg-4 mb-3 mb-md-0">
                                    <form action="{{ url()->current() }}" method="GET">
                                        ...
                                        <div class="input-group input-group-merge input-group-custom">
                                            ...
                                            <input id="datatableSearch_" type="search" name="search" class="form-control" placeholder="{{\App\CPU\translate('Search by orders id or transaction id')}}" aria-label="Search orders" value="{{ $search }}">
                                    ...
                                    <form action="{{ url()->current() }}" method="GET">
                                        <div class="d-flex justify-content-end align-items-center gap-10">
                                            <select class="form-control" name="status">
                                                ...
                                                @php
                                                    $arr = ['Pending', 'Approved', 'Rejected', 'Created', 'Already Have'];
                                                @endphp
                                                @foreach($arr as $skey => $stat)
                                                    <option class="text-capitalize" value="{{$skey}}" {{ $status == $skey ? 'selected' : '' }} >{{$stat}} </option>
                                                @endforeach
                                            </select>
                            ...
                            <table id="datatable" style="text-align: {{ Session::get('direction') === "rtl" ? 'right' : 'left' }};" class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100">
                                <thead class="thead-light thead-50 text-capitalize">
                                    <tr>
                                        ...
                                        <th>{{ \App\CPU\translate('sub_category_name') }}</th>
                                ...
                                <tbody>
                                    @foreach($transactions as $key=> $transaction)
                                        <tr>
                                            ...
                                            <td>{{ $transaction->sub_sub_category_name }}</td>
                                            <td class="text-center">
                                                <select class="form-control category-request-status" name="status" data-id="{{ $transaction->id }}">
                                                    <option value="0" selected disabled> {{ \App\CPU\translate('select_status') }}</option>
                                                    @foreach($arr as $skey => $stat)
                                                        <option class="text-capitalize" value="{{$skey}}" {{ $transaction->status == $skey ? 'selected' : '' }} >{{$stat}} </option>
                                                    @endforeach
                                                </select>
                                            </td>
                                        </tr>
...
@push('script_2')
    <script>
        ...
        $(document).on('change','.category-request-status', function() {
            let status = $(this).val()
            let id = $(this).data('id')

            $.ajax({
                type: "POST",
                url: "{{ route('admin.sellers.category-request.update') }}",
