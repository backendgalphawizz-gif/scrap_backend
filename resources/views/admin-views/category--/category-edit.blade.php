                        ...
                        <form action="{{route('admin.category.update',[$category['id']])}}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="{{ $category['parent_id']==0 || $category['position'] == 1 ? 'col-lg-6':'col-12' }}">
                                    ...
                                    @endforeach
                                    <div class="form-group {{ $category['parent_id']==0 ? 'd-none':'' }}">
                                        ...
                                        <select id="exampleFormControlSelect1" name="parent_id"
                                                class="form-control" {{ $category['parent_id']==0 ? '':'required' }}>
                                            <option value="" selected disabled>{{\App\CPU\translate('Select_main_category')}}</option>
                                            @foreach(\App\Model\Category::where(['position'=>$category['position'] - 1])->get() as $list)
                                                <option value="{{$list['id']}}" {{ $category['parent_id'] == $list['id'] ? 'selected' : '' }}>{{$list['name']}}</option>
                                            @endforeach
                                        </select>
                                    </div>
