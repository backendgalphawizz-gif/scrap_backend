    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 d-flex gap-2">
                ...
                {{\App\CPU\translate('Plan')}} {{\App\CPU\translate('List')}}
                        ...
                        <div class="row g-2 flex-grow-1">
                            ...
                            <div class="col-sm-4 col-md-6 col-lg-8 d-flex justify-content-end">
                                ...
                                <ul class="dropdown-menu">
                                    <li><a class="dropdown-item" href="{{ route('admin.plan.export') }}">Excel</a></li>
                                </ul>
                            </div>
                        </div>
                        <!-- End Row -->
                        ...
                        <div class="table-responsive">
                            <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                ...
                                <tr>
                                    ...
                                    <th>{{ \App\CPU\translate('Validity')}}</th>
                                    <th>{{ \App\CPU\translate('Price')}}</th>
                                    <th>{{ \App\CPU\translate('Plan for')}}</th>
                                ...
                                @foreach($br as $k=>$b)
                                    <tr>
                                        ...
                                        <td>
                                            <img class="rounded avatar-60" onerror="this.src='{{asset('public/assets/back-end/img/160x160/img2.jpg')}}'" src="{{asset('storage/app/public/brand')}}/{{$b['image']}}">
                                        </td>
                                        <td>{{ $b['title'] }}</td>
                                        <td>{{ \App\CPU\Helpers::get_time_type($b['time'], $b['type']) }}</td>
                                        <td>{{ \App\CPU\Helpers::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($b['price'])) }}</td>
                                        <td>{{ \App\CPU\Helpers::get_user_type_plan($b['user_type']) }}</td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-center gap-2">
                                                ...
                                                href="{{route('admin.plan.update',[$b['id']])}}">
                                                <i class="tio-edit"></i>
                                                </a>
...
@push('script')
    <script>
        ...
        $(document).on('change', '.status', function () {
            ...
            $.ajax({
                url: "{{route('admin.plan.status-update')}}",
                method: 'POST',
                data: {
                    id: id,
                    status: status
